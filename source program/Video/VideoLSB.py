...
class VideoUnLSB():
    ...
    def unstego_data(self, frame_count, mode, seed=0):
        ...
        print(f"Unstego mode: {mode}, Total needed frames: {total_needed_frames}, Seed: {seed}")
        ...
        if mode[0] == 1:
            ...
            for frame in frame_seq:
                ...
                path = os.path.join(os.getcwd(), "temp", f"{frame}.png")
                
                # Validate if frame exists
                if not os.path.exists(path):
                    print(f"Error: Frame {frame} not found at {path}")
                    continue
                
                target = os.path.join(os.getcwd(), "temp", f"{frame}.png")
                try:
                    f = FrameUnLSB(path, target)
                    if mode[1] == 1:
                        bits = f.unhide_data_random(34, seed)
                    else:
                        print("Sequential pixel mode")
                        bits = f.unhide_data(34)
                    data.extend(bits)
                    print(f"Data uncovered at frame {frame}, Total data length: {len(data)} bits")
        ...
        print(f"Finished extracting data. Total data length: {len(data)} bits")
        return data
